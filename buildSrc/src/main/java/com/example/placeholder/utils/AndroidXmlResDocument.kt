...
import com.example.placeholder.data.Constants.Companion.XML_RESOURCES_TAG
import com.example.placeholder.data.Constants.Companion.XML_STRING_TAG
...
class AndroidXmlResDocument(
    ...
    fun appendStringResource(stringResourceModel: StringResourceModel) {
        append(XmlUtils.stringResourceModelToElement(document, stringResourceModel))
    }
    ...
    fun getStringResourceList(): List<StringResourceModel> {
        ...
        for (it in 0 until strings.length) {
            strList.add(XmlUtils.nodeToStringResourceModel(strings.item(it)))
        }
        return strList
    }
